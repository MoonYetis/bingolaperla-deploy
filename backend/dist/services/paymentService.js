...
class PaymentService {
     ...
     */
    async getActiveBankConfigurations() {
        try {
            ...
            structuredLogger_1.logger.error('Error obteniendo configuraciones bancarias', error);
    ...
    async createDepositRequest(data) {
        try {
            ...
            const result = await prisma.$transaction(async (tx) => {
                ...
                const references = await referenceService_1.referenceService.generateBankingReference({
                    userId: data.userId,
                    amount: data.amount,
            ...
            const bankInstructions = this.prepareBankInstructions(data.paymentMethod, result.bankConfig, result.references.displayCode, data.amount);
            ...
            structuredLogger_1.logger.error('Error creando solicitud de depósito', error, data);
            throw error;
        }
    }
    /**
    ...
    async createWithdrawalRequest(data) {
        try {
            ...
            const balance = await walletService_1.walletService.getBalance(data.userId);
            ...
            const result = await prisma.$transaction(async (tx) => {
                // Generar referencia única
                const reference = await referenceService_1.referenceService.createPaymentReference({
                    userId: data.userId,
            ...
            structuredLogger_1.logger.error('Error creando solicitud de retiro', error, data);
            throw error;
        }
    }
    /**
     ...
     */
    async approveDeposit(depositRequestId, adminId, bankReference, adminNotes) {
        try {
            const result = await prisma.$transaction(async (tx) => {
            ...
            const creditResult = await walletService_1.walletService.creditPearls(result.depositRequest.userId, parseFloat(result.depositRequest.pearlsAmount.toString()), `Recarga aprobada - Ref: ${result.depositRequest.referenceCode}`, result.depositRequest.referenceCode, adminId);
            // Marcar referencia como utilizada
            const referenceValidation = await referenceService_1.referenceService.isValidReference(result.depositRequest.referenceCode);
            if (referenceValidation.valid && referenceValidation.reference) {
                await referenceService_1.referenceService.markAsUsed(referenceValidation.reference.id);
            }
            // Log de auditoría
            ...
            structuredLogger_1.logger.info('Depósito aprobado', {
                depositRequestId,
                userId: result.depositRequest.userId,
                amount: result.depositRequest.amount,
                adminId,
            ...
            structuredLogger_1.logger.error('Error aprobando depósito', error, { depositRequestId, adminId });
            throw error;
        }
    }
    /**
     ...
     */
    async rejectDeposit(depositRequestId, adminId, reason) {
        try {
            ...
            structuredLogger_1.logger.info('Depósito rechazado', {
                depositRequestId,
                userId: depositRequest.userId,
                adminId,
                reason
            ...
            structuredLogger_1.logger.error('Error rechazando depósito', error, { depositRequestId, adminId });
            throw error;
        }
    }
    /**
     ...
     */
    prepareBankInstructions(paymentMethod, bankConfig, referenceCode, amount) {
        if (!bankConfig) {
            // Para Yape/Plin
            return {
     ...
     */
    async createAuditLog(adminId, action, details) {
        try {
            ...
            structuredLogger_1.logger.error('Error creando log de auditoría', error);
        }
    }
}
