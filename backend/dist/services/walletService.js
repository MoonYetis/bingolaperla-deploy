...
class WalletService {
    ...
    async getByUserId(userId) {
        try {
            ...
            structuredLogger_1.logger.error('Error obteniendo billetera', error, { userId });
     ...
     */
    async createWallet(userId, initialBalance = 0.00) {
        try {
            ...
            structuredLogger_1.logger.info('Billetera creada', {
                walletId: wallet.id,
                userId,
                initialBalance
            });
            ...
            structuredLogger_1.logger.error('Error creando billetera', error, { userId });
            throw error;
        }
    }
    /**
    ...
    async getBalance(userId) {
        try {
            ...
            structuredLogger_1.logger.error('Error obteniendo balance', error, { userId });
            throw error;
        }
    }
    /**
     ...
     */
    async creditPearls(userId, amount, description, referenceId, adminId) {
        try {
            if (amount <= 0) {
            ...
            structuredLogger_1.logger.info('Perlas acreditadas', {
                userId,
                amount,
                newBalance: result.newBalance,
                transactionId: result.transaction.id
            ...
            structuredLogger_1.logger.error('Error acreditando Perlas', error, { userId, amount });
            throw error;
        }
    }
    /**
     ...
     */
    async debitPearls(userId, amount, description, type = 'CARD_PURCHASE', referenceId, toUserId) {
        try {
            if (amount <= 0) {
                throw new Error('El monto debe ser mayor a 0');
            }
            ...
            structuredLogger_1.logger.info('Perlas debitadas', {
                userId,
                amount,
                newBalance: result.newBalance,
                type,
            ...
            structuredLogger_1.logger.error('Error debitando Perlas', error, { userId, amount, type });
            throw error;
        }
    }
    /**
     ...
     */
    async transferPearls(fromUserId, toUserId, amount, description, commission = 2.50) {
        try {
            if (amount <= 0) {
                throw new Error('El monto debe ser mayor a 0');
            }
            ...
            structuredLogger_1.logger.info('Transferencia P2P completada', {
                fromUserId,
                toUserId,
                amount,
                commission,
            ...
            structuredLogger_1.logger.error('Error en transferencia P2P', error, { fromUserId, toUserId, amount });
            throw error;
        }
    }
    /**
    ...
    async getTransactionHistory(userId, options) {
        try {
            ...
            structuredLogger_1.logger.error('Error obteniendo historial', error, { userId });
     ...
     */
    async freezeWallet(userId, adminId, reason) {
        try {
            ...
            structuredLogger_1.logger.warn('Billetera congelada', { userId, adminId, reason });
            return wallet;
        }
        catch (error) {
            structuredLogger_1.logger.error('Error congelando billetera', error, { userId });
            throw error;
        }
    }
    /**
