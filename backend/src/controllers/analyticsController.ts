...
export class AnalyticsController {
  ...
  async getUserAnalytics(req: Request, res: Response) {
    try {
      ...
      const userAnalytics = await analyticsService.getUserAnalytics(limit);
  ...
  async getGameAnalytics(req: Request, res: Response) {
    try {
      ...
      const gameAnalytics = await analyticsService.getGameAnalytics(gameId);
  ...
  async getRealtimeMetrics(req: Request, res: Response) {
    try {
      ...
      const metrics = analyticsService.getRealtimeMetrics(hours);
   ...
   */
  async recordRealtimeMetrics(req: Request, res: Response) {
    try {
      const {
        activeUsers,
        activeGames,
      ...
      await analyticsService.recordRealtimeMetrics({
        activeUsers,
        activeGames,
        memoryUsage,
        cpuUsage,
      ...
      logger.error('Error recording realtime metrics:', error);
      res.status(500).json({
        success: false,
  ...
  async getTrends(req: Request, res: Response) {
    try {
      ...
      const trends = await analyticsService.getTrends(metric, days);
  ...
  async getDashboard(req: Request, res: Response) {
    try {
      const [
        ...
        analyticsService.getRealtimeMetrics(24), // Last 24 hours
        analyticsService.getTrends('users', 7),
        analyticsService.getTrends('games', 7),
        analyticsService.getTrends('revenue', 7)
      ]);
   ...
   */
  async getBingoKPIs(req: Request, res: Response) {
    try {
      ...
      const weeklyGames = await analyticsService.generateCustomReport(
        startOfWeek,
        today,
        ['games']
      );

      const monthlyRevenue = await analyticsService.generateCustomReport(
        startOfMonth,
        today,
        ['revenue']
      );
      ...
      logger.error('Error getting Bingo KPIs:', error);
      res.status(500).json({
        success: false,
